...
class ObjectStore:
    def __init__(self) -> None:
        ...
        self.schema: YAMLFragment | None = None

        self.object_constructors: dict[str, Callable[[dict[str, Any]], None]] = {
            "lecture": self.__init__lecture,
            "thesis": self.__init__thesis,
        }

        # We keep a pointer to the current set of referenced objects
        self.__referenced_objects: set[str] | None = None

    def set_referenced_objects(self, x: set[str]) -> None:
    ...
    def schema_for(self, obj: dict[str, Any]) -> dict[str, Any]:
        assert self.schema is not None
        return schema_for_obj(self.schema, obj)

    def permalink(self, data_dir: Any, name: str, page: Any) -> tuple[str, str]:
    ...
    def crawl_pages(
        ...
        for page in pages:
            ...
            if page.data["id"] in data_dir.data.get("permalinks", {}):
                assert "permalink.alias" not in page.data, (
                    f"Page {page.data['id']} ({page.data['__file__']}): global permalink alias conflicts with page-local permalink.alias. Remove either one"
                )
                page.data["permalink.alias"] = data_dir.data["permalinks"][
                    page.data["id"]
                ]
            ...
            if "permalink.alias" in page.data:
                ...
                assert alias not in aliases, (
                    f"Alias {alias} is duplicated: {page.data['__file__']}, {aliases[alias].data['__file__']}"
                )
                aliases[alias] = page
        ...
        for obj in data_dir.objects():
            assert obj["id"] not in objects, f"Duplicate Object ID ({obj['id']})"
            objects[obj["id"]] = obj
            ...
            if "object_aliases" in obj:
                for alias in obj["object_aliases"]:
                    assert alias not in objects, (
                        f"Alias (object_aliases) {alias} is duplicated"
                    )
                    objects[alias] = obj
        ...
        for obj in list(objects.values()):
            if self.isA(obj, "bibtex"):
                ...
                def in_prjs(prj: str) -> bool:
                    if prj == "":
                        return False
                    if prj not in [item["id"] for item in self.object_list("project")]:
                        logging.warning("Project %s unknown, ignoring", prj)
                        return False
                    return True
            ...
            for obj in objects.values():
                if "depends" in obj:
                    ...
                    for other in list(deps):
                        assert other in objects, f"Dependency {other} is unknown"
        ...
        logging.info("%d pages, %d objects", len(page_objects), len(self.objects))
    ...
    def has_menu_children(self, elem: Any) -> bool:
        resolved = self.deref(elem)
        assert resolved is not None
        entries = resolved.get("menu") or resolved.get("children", [])
        for p in entries:
            p = self.deref(p)
    ...
    def is_child_of(self, elem: Any, parent: Any) -> bool:
        p = self.deref(parent)
        x = self.deref(elem)
        while x:
            if x == p:
    ...
    def teaching_sose(self, elem: str) -> bool:
        return elem.startswith("lehre-ss")

    def teaching_wise(self, elem: str) -> bool:
        return elem.startswith("lehre-ws")

    @staticmethod
    def __init__lecture(obj: dict[str, Any]) -> None:
    ...
    def __init__thesis(obj: dict[str, Any]) -> None:
        ...
        if "thesis-end" in obj:
            ...
            obj["thesis-year"] = Path(obj["__file__"]).parent.name

    def get_submenu(self, page: Any) -> list[Any]:
    ...
    def deref(self, elem: Any, fail: bool = True) -> dict[str, Any] | None:
        ...
        if type(elem) is dict:
            ...
            raise RuntimeError(f"Object '{elem}' not found")

    def isA(self, obj: dict[str, Any], Type: str) -> bool:
    ...
    def object_unique(self, objs: list[Any]) -> list[Any]:
        ids = set()
        ret = []
        for obj in objs:
            if type(obj) is str:
    ...
    def sorted(self, elem: list[dict[str, Any]], **kwargs: Any) -> list[dict[str, Any]]:
        def sort_key(x: dict[str, Any]) -> Any:
            ...
            if self.isA(x, "thesis"):
                return (
                    str(x.get("thesis-end", x["thesis-year"] + "-12-31")) + x["title"]
                )
            if "id" in x:
                return x["id"]
            return str(x)
